...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        """
        neighbors_list = np.zeros([num_neighbors, 2])

        #alpha: vetor de ângulos para um Hill Climbing 8-conectado
        alpha = np.zeros(num_neighbors)
        i = 0
        while i < num_neighbors:
            alpha[i] = 2*i*pi/num_neighbors
            neighbors_list[i][0] = theta[0] + delta * cos(alpha[i])
            neighbors_list[i][1] = theta[1] + delta * sin(alpha[i])
            i += 1
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        knock_house = random.randint(0, num_neighbors-1)
        return neighbors_list[knock_house]
